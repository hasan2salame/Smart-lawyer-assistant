...
RESULTS_FILE = Path(__file__).parent / "results" / "retrieval_eval2.json"
K_VALUES     = [1, 3, 6, 10]
...
def _rrf_graph(retriever: HybridRetriever, query: str) -> list[dict]:
    """
    Run RRF + Graph pipeline (before reranker).
    NOTE: هذا لا يعكس الـ pipeline الإنتاجي الكامل (يفتقد الـ reranker وadaptive_filter).
    يُستخدم للـ ablation study فقط. للقياس الإنتاجي الحقيقي استخدم _full_pipeline().
    """
    fused = rrf_merge(
...
def _full_pipeline(retriever: HybridRetriever, query: str) -> list[dict]:
    """Full production pipeline including reranker + adaptive_filter."""
