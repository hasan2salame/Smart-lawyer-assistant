...
# -*- coding: utf-8 -*-
"""
scripts/upload_laws.py
Upload Syrian Personal Status Law articles to Qdrant.
...
INPUT_FILE      = "data/processed/laws.json"
COLLECTION_NAME = "legal_laws"
BATCH_SIZE      = 30
SLEEP_SECS      = 35

# ── LlamaIndex — no chunking (one article = one vector) ────────────────────
...
Settings.llm          = None
Settings.chunk_size   = 8192   # large enough to keep each article as one node
Settings.chunk_overlap = 0

qdrant = QdrantClient(url=QDRANT_URL, api_key=QDRANT_API_KEY, timeout=120)

# ── Drop and recreate the collection cleanly ────────────────────────────────
...
for item in data:
    ...
    # Embedding text: concise — only what aids semantic retrieval
    embedding_text = (
    ...
    docs.append(Document(
        ...
        # prevent LlamaIndex from splitting metadata into chunks
...
for i in range(0, total, BATCH_SIZE):
    ...
    for attempt in range(3):
        try:
            ...
            print(f"  batch {batch_num}/{total_batches} — {uploaded}/{total} points uploaded ({info.points_count} in collection}")
            break
        except Exception as e:
            wait = 15 * (attempt + 1)
            print(f"  attempt {attempt+1}: {str(e)[:100]} — waiting {wait}s")
            time.sleep(wait)
    else:
...
info = qdrant.get_collection(COLLECTION_NAME)
print(f"\nlegal_laws — {info.points_count} points")
print(f"   Expected: {total} (one point per article)")
