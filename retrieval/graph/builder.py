...
بناء شبكة العلاقات بين المواد القانونية

يُنشئ graph.json من Qdrant ويحفظه في data/graph.json.
يُشغَّل مرة واحدة (أو عند تحديث البيانات).

أنواع الروابط المبنية:
  [1] Article refs  : مادة تذكر مادة أخرى صراحةً في نصها
  [2] Topic mapping : صياغات fonon مرتبطة بمواد موضوعية
  [3] Adjacency     : مواد متجاورة رقمياً (article_num ± 1)

قرارات تصميمية مبنية على فحص البيانات الفعلية:
  - bab/fasl metadata معطوبة (كل المواد في مجموعة واحدة)
    → استُبدلت بالجوار الرقمي المباشر
  - "دعوى" محذوفة من TOPIC_OSOUL
    → تظهر في 26/28 قالب، لا قيمة دلالية
  - cross-law refs مقيّدة بذكر صريح لاسم القانون
    → يمنع الروابط المزيفة
"""
...
from config import COL_LAWS, COL_OSOUL, COL_FONON, GRAPH_FILE, DATA_PROCESSED
...
def fetch_all(client: QdrantClient, collection: str) -> list:
    """جلب كل النقاط من collection بالتصفح الدفعي."""
...
def make_key(payload: dict) -> str:
    """يولّد مفتاحاً طبيعياً من الـ payload."""
...
def extract_refs(text: str) -> list:
    ...
    يتطلب سياقاً صريحاً (المادة/م.) لتجنب false positives.
    """
    nums = set()
...
def add_edge(graph: dict, src: str, dst: str):
    """إضافة رابط أحادي الاتجاه مع تجنب التكرار."""
...
# ملاحظة: "دعوى" محذوفة — تظهر في 26/28 قالب فتولّد روابط مزيفة
TOPIC_OSOUL = {
...
def build_graph(client: QdrantClient) -> dict:
    """يبني الشبكة الكاملة ويُرجعها كـ dict."""
    graph = {}
