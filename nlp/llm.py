...
# ── Client-data heuristic (zero API cost) ────────────────────────────────
import re as _re

_CLIENT_DATA_RE = _re.compile(
    r"""
    \d{4}[/\-]\d{1,2}[/\-]\d{1,2}   # تاريخ YYYY-MM-DD
    | \d{1,2}[/\-]\d{1,2}[/\-]\d{4}  # تاريخ DD-MM-YYYY
    | \b\d{5,}\b                        # رقم هوية أو رقم طويل
    | (?:السيد|السيدة|الأستاذ|الأستاذة)\s+\w+   # لقب + اسم
    | (?:موكل|موكلة|المدعي|المدعى\s+عليه|الزوج|الزوجة)\s+[\w/]+  # دور + اسم
    | (?:اسمه|اسمها|اسم\s+موكل)\s*[:\s]\s*\w+  # اسمه: ...
    """,
    _re.VERBOSE | _re.UNICODE,
)

def _has_client_data(query: str) -> bool:
    """
    Heuristic: هل الاستعلام يحتوي على بيانات موكل محددة؟
    تعمل بدون API call — تستخدم regex للكشف عن أسماء وتواريخ وأرقام.
    """
    return bool(_CLIENT_DATA_RE.search(query))
...
def _detect_fill_scenario(
    ...
    المرحلة 1 (بدون API): هل في بيانات موكل في الاستعلام؟
    المرحلة 2 (LLM فقط عند الغموض): fill_history أم fill_inline؟

    Returns
    -------
    "fill_history"  — المحامي يزود بيانات لصياغة سابقة
    "fill_inline"   — المحامي دمج بيانات الموكل في طلب جديد
    "none"          — لا يوجد سيناريو تعبئة
    """
    has_prev = bool(last_template and history)
    has_data = _has_client_data(query)

    # لا بيانات موكل ولا تاريخ سابق → طلب صياغة جديد خام
    if not has_data and not has_prev:
        return "none"

    # بيانات موجودة لكن لا قالب سابق → fill_inline مباشرة (بدون LLM)
    if has_data and not has_prev:
        return "fill_inline"

    # قالب سابق موجود → LLM يحدد هل المحامي يُكمّل ذلك القالب أم يطلب جديداً
    last_msg = _last_assistant_msg(history)
    prompt = (
        f"عرض النظام السابق (مختصر):\n{last_msg[:300]}\n\n"
        f"رسالة المحامي الجديدة:\n{query}\n\n"
        "هل المحامي يزود بيانات لتعبئة الصياغة السابقة؟\n"
        "أجب بـ 1 إذا نعم (fill_history)، أو 2 إذا يطلب صياغة جديدة مع بيانات (fill_inline)، أو 3 إذا لا شيء.\n"
        "رقم واحد فقط."
    )
    answer = _call(
        system="أجب برقم واحد فقط: 1 أو 2 أو 3.",
        query=prompt,
        history=[],
        model=MODEL_FAST,
        max_tokens=3,
    ...
    if "2" in answer:
        return "fill_inline"
    # إذا في قالب سابق وبيانات موجودة والـ LLM قال 3 → افتراضياً fill_history
    if has_prev and has_data:
        return "fill_history"
    return "none"
...
def _handle_fill(query: str, template: dict, history: list, max_tokens: int = 2048) -> dict:
    ...
    message = _call(
        ...
        model=MODEL_LEGAL,   # 70b — أدق في التعبئة وأقل هلوسة من 8b
        max_tokens=max_tokens,
        temperature=0.05,
    )
    return {
...
def _append_sources(message: str, articles: list) -> str:
    ...
    if not articles or "**المصادر:**" in message or "---\n**المصادر" in message:
        return message
...
def process(
    ...
    # Scenario detection:
    # - fill_inline: يُكشف حتى بدون history عبر heuristic (بدون API cost)
    # - fill_history: يحتاج last_template أو history
    # المرحلة الأولى من _detect_fill_scenario تعمل بـ regex فقط (صفر تكلفة)
    if has_tmpl:
